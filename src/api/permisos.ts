...
export const PermisosApi = {
  async getAll(): Promise<PermisoRolEntidad[]> {
    const response = await apiCall<ApiEnvelope<PermisoRolEntidad[]> | PermisoRolEntidad[]>(
  ...
  async update(id: number, payload: UpdatePermisoRolEntidadPayload): Promise<void> {
  ...
  async createRol(payload: { nombre: string; descripcion?: string }): Promise<void> {
    await apiCall('/rol', {
      method: 'POST',
      body: JSON.stringify(payload),
      skipJson: true,
  ...
  async updateRol(id: number, payload: { nombre: string; descripcion?: string }): Promise<void> {
  ...
  async getPermisos(): Promise<Permiso[]> {
    const response = await apiCall<ApiEnvelope<Permiso[]> | Permiso[]>('/permiso');
    return asArray(response);
  },

  async getEntidades(): Promise<Entidad[]> {
    const response = await apiCall<ApiEnvelope<Entidad[]> | Entidad[]>('/entidad');
    return asArray(response);
  },
};
