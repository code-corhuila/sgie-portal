...
export const EquiposApi = {
  async getAll(): Promise<EquipoSummary[]> {
    const response = await apiCall<ApiEnvelope<EquipoSummary[]> | EquipoSummary[]>('/equipo/equipo-instalacion');
    return asArray(response);
  },

  async searchByCodigo(codigo: string): Promise<EquipoSummary[]> {
    const response = await apiCall<ApiEnvelope<EquipoSummary[]> | EquipoSummary[]>(
  ...
  async updateEquipo(idEquipo: number, payload: UpdateEquipoPayload): Promise<void> {
  ...
  async getCategorias(): Promise<CategoriaEquipo[]> {
    const response = await apiCall<ApiEnvelope<CategoriaEquipo[]> | CategoriaEquipo[]>('/categoria-equipo');
    return asArray(response);
  },
  ...
  async getTipos(): Promise<TipoEquipo[]> {
    const response = await apiCall<ApiEnvelope<TipoEquipo[]> | TipoEquipo[]>('/tipo-equipo');
    return asArray(response);
  },
  ...
  async getInstalaciones(): Promise<InstalacionOption[]> {
    const response = await apiCall<ApiEnvelope<InstalacionOption[]> | InstalacionOption[]>('/instalacion');
    return asArray(response);
  },
};
